...
class QueryParser:
    ...
    def _expand_acronyms(self, text: str, resolver: Any) -> str:
        ...
        def replace(match: re.Match) -> str:
            ...
            if candidates:
                shortest = min(
                    candidates, key=lambda name: (len(name.split()), len(name))
                )
    ...
    def extract(
        ...
        if not _skip_paren:
            ...
            if not paren_warnings:
                for open_idx, close_idx, inner_text in raw_groups:
                    ...
                    if operator is None and re.search(
                        r"\band\b|\bor\b", inner_text, re.IGNORECASE
                    ):
                        paren_warnings.append(
                ...
                for open_idx, close_idx, _ in reversed(raw_groups):
                    working_query = (
                        working_query[:open_idx] + working_query[close_idx + 1 :]
                    )
                working_query = re.sub(r"\s+", " ", working_query).strip()
                working_query = self.engine.strip_dangling_logical_operators(
                    working_query
                )
        ...
        global_age_constraints, _ = self.engine.extract_age_constraints(
            working_query, "query"
        )
        ...
        for candidate in candidates:
            ...
            candidate_normalised = self.engine.apply_mappings(
                candidate_normalised, "bmi", warnings
            )
        ...
        for candidate in candidates:
            ...
            demographic_only_for_scope = not self.engine.has_non_demographic_content(
        ...
        for candidate in candidates:
            ...
            is_gender_only = self.engine.has_demographic_concept(
                candidate_normalised
            ...
            if not matches:
                ...
                unmatched_text = candidate_clean or candidate

                warnings.append(
                    f'We cannot find any matching concepts for your term "{unmatched_text}".'
                )
                ...
                entities.append(
                    {
                        ...
                        "end": end_idx,
                        "negated": negated,
                        "age_constraints": entity_age_constraints
                        if entity_age_constraints is not None
                        else [],
